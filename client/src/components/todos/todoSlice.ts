import { createSlice, createAsyncThunk, createEntityAdapter, PayloadAction, Action, Dispatch, Middleware, isFulfilled } from "@reduxjs/toolkit";
...
export const getTodosAsync = createAsyncThunk("todos/getTodos", async (uid: number | null) => {
...
export const createTodoAsync = createAsyncThunk("todos/createTodo", async ({ title, description }: CreateTodoInput) => {
  const response = await createTodo(title, description);
  return response;
});

export const updateTodoAsync = createAsyncThunk("todos/updateTodoAsync", async ({ id, ...patchTodo }: Partial<Todo>) => {
  const response = await updateTodo(id as number, patchTodo);
  return response;
});

export const deleteTodosAsync = createAsyncThunk("todos/deleteTodosAsync", async (ids: number[]) => {
