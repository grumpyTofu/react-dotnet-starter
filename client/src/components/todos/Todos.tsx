...
const Todos: React.FC<TodosProps> = ({}) => {
  ...
  useEffect(() => {
    if (todos && todos.length === 0) {
      dispatch(getTodosAsync(user ? (user.id as number) : null));
    }
  }, []);
  ...
  return (
    <Grid container spacing={2}>
      ...
      {todos && todos.length > 0 && (
        <>
          {todos.map((todo) => (
            <TodoCard todo={todo} state={todoState} setState={setTodoState} key={`TodoCard_${todo.id}`} />
          ))}
        </>
      )}
      <TodoDialog
        ...
        todo={todoState.todos.length === 1 ? todosSelector.selectById(store.getState(), todoState.todos[0]) : undefined}
      />
    </Grid>
  );
};
